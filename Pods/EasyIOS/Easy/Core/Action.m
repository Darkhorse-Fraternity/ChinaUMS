...
-(void)checkCode:(Request *)msg{
    if (msg.needCheckCode) {
        ...
        if([msg.output objectAtPath:[Action sharedInstance].CODE_KEY] && [[msg.output objectAtPath:[Action sharedInstance].CODE_KEY]  isEqualToString:@"OK"]){
            [self success:msg];
        }else{
            [self error:msg];
        }
...
-(AFHTTPRequestOperation *)POST1:(Request *)msg{
    NSString *url = @"";
    NSDictionary *requestParams = nil;
    if(msg.STATICPATH.isNotEmpty){
        url = msg.STATICPATH;
    ...
    msg.url = [NSURL URLWithString:url];
...
//    manager.requestSerializer.HTTPShouldHandleCookies = NO;
    ...
    [self sending:msg];
    @weakify(self);
    NSURLSessionDataTask *task = [manager POST:url parameters:requestParams  success:^(NSURLSessionDataTask *operation, NSDictionary* jsonObject) {
        @strongify(self);
        if( _cacheEnable){
            [[TMCache sharedCache] setObject:jsonObject forKey:msg.cacheKey block:^(TMCache *cache, NSString *key, id object) {
                EZLog(@"%@ has cached",url);
